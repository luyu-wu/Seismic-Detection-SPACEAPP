...
for i in range(length):
    ...
    if os.path.exists(csv_file):
        ...
        algo_spot = algorithm.findTime(
            csv_times, csv_data, highpass=highPass, showgraphs=showGraphs
        )

        if abs(algo_spot - arrival_time) > 1e3 and drawExceptions:
            ...
            temp_ax.axvline(
                x=algo_spot, c="blue", linestyle="dashed", label="Algorithm Arrival"
            )
            temp_ax.legend()
