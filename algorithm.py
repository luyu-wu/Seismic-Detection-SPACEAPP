...
def showGraph(time, vel):
    ...
    rtm_rat_ax.fill_between(
        time_rtm, rtm_ratio_small - rtm_ratio_large, 0, alpha=0.3, facecolor="#089FFF"
    )

    rtm_rat_ax.plot(time_rtm, rtm_ratio_small, label="Small Box")
    rtm_rat_ax.fill_between(
        time_rtm, rtm_ratio_small, 0, alpha=0.3, facecolor="#FF8419"
    )

    rtm_rat_ax.plot(time_rtm, rtm_ratio_large, label="Large Box")
    rtm_rat_ax.fill_between(
        time_rtm, rtm_ratio_large, 0, alpha=0.3, facecolor="#65BC85"
    )
